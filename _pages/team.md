---
title: ""
permalink: /Team/
author_profile: false
---

<h1 style="margin-left: -50px;">Team</h1>

{% include base_path %}

<section id="main">
	<div class="inner">
		<h2 style="margin-left: -50px;">Principal Investigator</h2>
			<div style="font-size:12pt;line-height:1.5;margin-left: -50px;">
				<div class="row" style="align-items: center;">
					<div style="width:25%; padding:10px;">
						<img src="../images/team/Tianyu_Wang.jpg" width="90%" style="border-radius: 5px;"/>
					</div>
					<div style="width:75%; padding:10px;">
						<span style="font-size:14pt; font-weight:bold;">Tianyu Wang</span><br />
						Assistant Professor<br />
						Ph.D. in Applied Physics, Cornell University <br />
						<a href="https://www.bu.edu/eng/academics/departments-and-divisions/electrical-and-computer-engineering/" target="_blank" style="color:inherit; text-decoration:underline;">Department of Electrical & Computer Engineering</a><br />
						<a href="https://www.bu.edu/photonics/" target="_blank" style="color:inherit; text-decoration:underline;">Photonics Center</a> | <a href="https://www.bu.edu/neurophotonics/people/faculty/" target="_blank" style="color:inherit; text-decoration:underline;">Neurophotonics Center (NPC)</a> | <a href="https://www.bu.edu/cise/" target="_blank" style="color:inherit; text-decoration:underline;">Center for Information & Systems Engineering (CISE)</a><br /><br />
						Email: wangty_AT_bu.edu<br />
						<a href="https://scholar.google.com/citations?user=_mzQX1EAAAAJ&hl=en" target="_blank">Google Scholar</a> | <a href="https://www.linkedin.com/in/tianyu-wang-44247040" target="_blank">LinkedIn</a> | <a href="/files/CurriculumVitae_Dec_2025.pdf" target="_blank">CV</a>
					</div>
				</div>
			</div><br clear="all" />

		<h2 style="margin-left: -50px;">Postdoctoral Researchers</h2>
			<div style="font-size:12pt;line-height:1.5;margin-left: -50px;">
				<div class="row" style="align-items: center;">
					<div style="width:25%; padding:10px;">
						<img src="../images/team/Weiru_Fan.png" width="100%" style="border-radius: 5px;"/>
					</div>
					<div style="width:75%; padding:10px;">
						<span style="font-size:14pt; font-weight:bold;">Weiru Fan</span><br />
						Ph.D. in Physics, Zhejiang University, China <br /><br />
						<b>Research interests:</b> Optical computing; Computational imaging; Light field modulation.<br /><br />
						<a href="https://scholar.google.com/citations?user=X7_uE2sAAAAJ&hl=zh-TW" target="_blank">Google Scholar</a>
					</div>
				</div>
			</div><br clear="all" />

		<h2 style="margin-left: -50px;">PhD Students</h2>
			<div style="font-size:12pt;line-height:1.5;margin-left: -50px;">
				<div class="row" style="align-items: center;">
					<div style="width:25%; padding:10px;">
						<img src="../images/team/Dingning_Li.jpg" width="100%" style="border-radius: 5px;"/>
					</div>
					<div style="width:75%; padding:10px;">
						<span style="font-size:14pt; font-weight:bold;">Dingning Li</span><br />
						Joined Lab in 2024<br />
						B.S. in Material Sciences/Physics, Duke Kunshan University, China <br /><br />
						<b>Research interests:</b> Photonic Integrated Circuits <br /><br />
						<a href="https://scholar.google.co.uk/citations?user=2y4uRFgAAAAJ&hl=en" target="_blank">Google Scholar</a> | <a href="https://www.linkedin.com/in/dingning-li-02137516a/" target="_blank">LinkedIn</a> 
					</div>
				</div>
			</div>

			<div style="font-size:12pt;line-height:1.5;margin-left: -50px;">
				<div class="row" style="align-items: center;">
					<div style="width:25%; padding:10px;">
						<img src="../images/team/Zipei_Wu.jpeg" width="100%" style="border-radius: 5px;"/>
					</div>
					<div style="width:75%; padding:10px;">
						<span style="font-size:14pt; font-weight:bold;">Zipei Wu</span><br />
						Joined Lab in 2025<br />
						B.S. in Optoelectronics, Shenzhen University, China <br /><br />
						<b>Research interests:</b> Biomedical Optics<br /><br />
						<a href="https://www.linkedin.com/in/zipei-wu-694b66262/" target="_blank">LinkedIn</a> | <a href="https://garyhost0630.github.io" target="_blank">Personal Website</a>
					</div>
				</div>
			</div>

			<div style="font-size:12pt;line-height:1.5;margin-left: -50px;">
				<div class="row" style="align-items: center;">
					<div style="width:25%; padding:10px;">
						<img src="../images/team/Tianhao_Jiang.jpeg" width="100%" style="border-radius: 5px;"/>
					</div>
					<div style="width:75%; padding:10px;">
						<span style="font-size:14pt; font-weight:bold;">Tianhao Jiang</span><br />
						Joined Lab in 2025<br />
						M.S. in Electrical and Computer Engineering, University of Michigan, Ann Arbor <br />
						B.S. in Physics, University of Washington, Seattle <br /><br />
						<b>Research interests:</b> Optical Instrumentation; Physical Modeling. <br /><br />
					</div>
				</div>
			</div><br clear="all" />


		<h2 style="margin-left: -50px;">Master's Students</h2>
			<div style="font-size:12pt;line-height:1.5;margin-left: -50px;">
				<div class="row" style="align-items: center;">
					<div style="width:25%; padding:10px;">
						<img src="../images/team/Mevlut_Gundogan.png" width="100%" style="border-radius: 5px;"/>
					</div>
					<div style="width:75%; padding:10px;">
						<span style="font-size:14pt; font-weight:bold;">Mevlut Gundogan</span><br />
						Joined Lab in 2026<br />
						B.S. in Electrical Engineering at Koç University, Istanbul, Turkey <br /><br />
						<b>Research interests:</b> Photonic Integrated Circuits and AI Accelerator Hardware. <br /><br />
						<a href="https://www.linkedin.com/in/mevlut-can-gundogan/" target="_blank">LinkedIn</a> | <a href="https://github.com/mgundogan20/" target="_blank">GitHub</a>
					</div>
				</div>
			</div><br clear="all" />

		<h2 style="margin-left: -50px;">Undergraduate Students</h2>
			<div style="font-size:12pt;line-height:1.5;margin-left: -50px;">
				<div class="row" style="align-items: center;">
					<div style="width:25%; padding:10px;">
						<img src="../images/team/Zitong_He.jpeg" width="60%" style="border-radius: 5px;"/>
					</div>
					<div style="width:75%; padding:10px;">
						<span style="font-size:14pt; font-weight:bold;">Zitong He</span><br />
						Class of 2026 | Computer Engineering <br />
						Joined Lab in 2024 <br />
						UROP Summer 2025, Fall 2025 <br /><br />
						<b>Research interests:</b> Digital System Design (FPGA) <br />
					</div>
				</div>
			</div>

			<div style="font-size:12pt;line-height:1.5;margin-left: -50px;">
				<div class="row" style="align-items: center;">
					<div style="width:25%; padding:10px;">
						<img src="../images/team/Esther_Xu.jpeg" width="60%" style="border-radius: 5px;"/>
					</div>
					<div style="width:75%; padding:10px;">
						<span style="font-size:14pt; font-weight:bold;">Esther Xu</span><br />
						Class of 2026 | Electrical Engineering with a minor in Physics <br />
						Joined Lab in 2025 <br />
						UROP Spring 2026 <br /><br />
						<b>Research interests:</b> Optics and Lasers <br />
					</div>
				</div>
			</div><br clear="all" />

		<h2 style="margin-left: -50px;">Visitors</h2>


		<h2 style="margin-left: -50px;">Alumni</h2>
			<div style="font-size:12pt;line-height:1.5;margin-left: -50px;">
				<p><span style="font-weight:bold;">Nolan Vild</span>, Master's Student — M.S., 2025 <span style="color:#bb0000;"> >>> </span>   <br />
				Legacy: Founding member of the lab; designed and built the first optical neural network based flow cytometer. </p>
			</div>

			<div style="font-size:12pt;line-height:1.5;margin-left: -50px;">
				<p><span style="font-weight:bold;">Kit Ng</span>, Master's Student — M.S., 2026  <span style="color:#bb0000;"> >>> </span> RF Circuit Designer <br />
				Legacy: Founding member of the lab; set up the electronics workstation; designed and fabricated the lab’s first circuit board for photodiode arrays; constructed a microfluidic system.  </p>
			</div>

			<div style="font-size:12pt;line-height:1.5;margin-left: -50px;">
				<p><span style="font-weight:bold;">Yongning Young Ma</span>, Master's Student — M.S., 2025 <span style="color:#bb0000;"> >>> </span> Digital Design Engineer, Texas Instruments<br />
				Legacy: Founding member of the lab; professionally set up and configured the lab server; initiated and led FPGA programming projects. </p>
			</div>

			<div style="font-size:12pt;line-height:1.5;margin-left: -50px;">
				<p><span style="font-weight:bold;">Andrew Sasamori</span>, Undergraduate Researcher — B.S., 2025<br />
				Legacy: Independently studied and elucidated the principle of Kolmogorov-Arnold Networks (KAN) to the lab.</p>
			</div>

			<div style="font-size:12pt;line-height:1.5;margin-left: -50px;">
				<p><span style="font-weight:bold;">Liam Deighton</span>, Undergraduate Researcher 2025 — Undergraduate at Washington University in St. Louis<br />
				Legacy: Independently trained machine-learning models predicting visual stimuli based on calcium neuronal imaging data in the mouse brain. </p>
			</div>

			<div style="font-size:12pt;line-height:1.5;margin-left: -50px;">
				<p><span style="font-weight:bold;">Nathan Cao</span>, Undergraduate Researcher (NSF REU 2025) — Undergraduate at Rice University<br /></p>
			</div>

			<div style="font-size:12pt;line-height:1.5;margin-left: -50px;">
				<p><span style="font-weight:bold;">Shuwen Xue</span>, Undergraduate Researcher (Summer 2025) — Undergraduate at Zhejiang University<br />
				Legacy: Benchmarked different spatial light modulators and piezoelectric motors; worked on multi-plane light conversion for generating optical angular momentum beams; investigated AI tools for experiment automation.</p>
			</div>


	</div>
</section>
